...
public class EigenschaftsStatistik extends Statistik {
	...
	public void errechne(Dokument dokument) {
		...
		for (Absatz absatz : dokument.getAbsaetze()) {
			int endIndex = Math.min(25, absatz.getText().length()-1); //zeige max. die 25 ersten Zeichen eines Absatzes
			sb.append("--").append(absatz.getText().substring(0, endIndex)).append("\n");
			...
			for(Satz satz : absatz.getSaetze()) {
				endIndex = Math.min(15, satz.getText().length()-1); //zeige max. die 15 ersten Zeichen eines Satzes
				sb.append("----").append(satz.getText().substring(0, endIndex)).append("\n");
